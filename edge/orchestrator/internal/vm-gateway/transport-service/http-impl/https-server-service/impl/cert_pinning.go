...
// VerifyClientCertificatePinning verifies that the client's CA certificate matches the pinned fingerprint.
// This is called during the TLS handshake to ensure we're accepting connections from the expected client.
func VerifyClientCertificatePinning(
	...
	return func(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
		...
		if config.EdgeCAFingerprint == "" {
			logger.Debug("Certificate pinning enabled but no Edge CA fingerprint configured, skipping pinning check")
			return nil
		}
		...
		expectedFingerprint := config.EdgeCAFingerprint
		...
		if fingerprintLower != expectedFingerprintLower {
			logger.Error("Certificate pinning verification failed for client",
		...
		logger.Debug("Certificate pinning verification succeeded for client",
...
// SetupServerCertificatePinning configures TLS to use certificate pinning verification for client certificates.
// This should be called when creating the TLS config for the HTTPS server.
func SetupServerCertificatePinning(
	tlsConfig *tls.Config,
	pinningConfig *vmgatewaytypes.CertificatePinningConfig,
	logger *zap.Logger,
) {
	if !pinningConfig.PinningEnabled {
		logger.Debug("Certificate pinning is disabled for server")
		return
	}

	if pinningConfig.EdgeCAFingerprint == "" {
		logger.Debug("Certificate pinning enabled but no Edge CA fingerprint configured for server")
		return
	}
	...
	tlsConfig.VerifyPeerCertificate = func(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
		...
		verifyFunc := VerifyClientCertificatePinning(pinningConfig, logger)
		return verifyFunc(rawCerts, verifiedChains)
	}

	logger.Info("Certificate pinning configured for HTTPS server",
		zap.String("edge_ca_fingerprint", pinningConfig.EdgeCAFingerprint))
}
