...
- `EDGE_AI_MIN_NORMAL_SNAPSHOTS` - Minimum normal snapshots required per camera (default: 50, must be > 0)
...
- `EDGE_STORAGE_SCREENSHOT_RETENTION_DAYS` - Screenshot retention in days (0 = no automatic deletion)
- `EDGE_STORAGE_SCREENSHOT_MAX_SIZE_MB` - Maximum size per screenshot in MB (0 = no limit)
- `EDGE_STORAGE_SCREENSHOT_MAX_TOTAL_SIZE_GB` - Maximum total size for all screenshots in GB (0 = no limit)
