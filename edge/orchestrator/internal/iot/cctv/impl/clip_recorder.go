...
func NewClipRecorder(ffmpeg *FFmpegWrapper, config ClipRecorderConfig, log *zap.Logger) (*ClipRecorder, error) {
...
func (r *ClipRecorder) StopRecording(cameraID string) (*ClipMetadata, error) {
	...
	if err != nil {
		r.logger.Warn("Failed to generate clip metadata", zap.Error(err), zap.String("camera_id", cameraID))
	...
	r.logger.Info("Stopped recording",
		zap.String("camera_id", cameraID),
		zap.String("output", recording.OutputPath),
...
func (r *ClipRecorder) StopAll() map[string]*ClipMetadata {
	...
	for _, cameraID := range cameraIDs {
		...
		if err != nil {
			r.logger.Warn("Failed to stop recording", zap.Error(err), zap.String("camera_id", cameraID))
			continue
		}
...
func (r *ClipRecorder) monitorRecording(recording *Recording) {
	...
	r.logger.Info("Recording completed",
		zap.String("camera_id", recording.CameraID),
		zap.String("output", recording.OutputPath),
	)
}
