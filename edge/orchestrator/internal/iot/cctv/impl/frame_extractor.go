...
type FrameExtractor struct {
	...
	frameBuffer      chan *cctvtypes.Frame
	bufferSize       int
	extractInterval  time.Duration
	preprocessConfig PreprocessConfig
	onFrame          func(*cctvtypes.Frame) // Callback for extracted frames
	mu               sync.RWMutex
	running          bool
	ctx              context.Context
...
func NewFrameExtractor(
	...
	return &FrameExtractor{
		logger:           log,
		ffmpeg:           ffmpeg,
		frameBuffer:      make(chan *cctvtypes.Frame, bufferSize),
...
func (e *FrameExtractor) Start(input string, cameraID string) error {
	...
	e.logger.Info("Frame extractor started",
		...
		zap.Duration("interval", e.extractInterval),
		zap.Int("buffer_size", e.bufferSize),
	)
...
func (e *FrameExtractor) extractFrames(input string, cameraID string) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				e.logger.Warn("Failed to extract frame", zap.Error(err), zap.String("camera_id", cameraID))
				continue
			}
			...
			if err != nil {
				e.logger.Warn("Failed to preprocess frame", zap.Error(err), zap.String("camera_id", cameraID))
				continue
			}
			...
			default:
				...
				e.logger.Debug("Frame buffer full, dropped oldest frame", zap.String("camera_id", cameraID))
			}
...
func (e *FrameExtractor) extractSingleFrame(input string, cameraID string) (*cctvtypes.Frame, error) {
	// Use FFmpeg wrapper's CaptureFrameJPEG method
	frameData, err := e.ffmpeg.CaptureFrameJPEG(e.ctx, input, e.preprocessConfig.Quality)
	if err != nil {
...
func (e *FrameExtractor) preprocessFrame(frame *cctvtypes.Frame) (*cctvtypes.Frame, error) {
...
func (e *FrameExtractor) GetFrameBuffer() <-chan *cctvtypes.Frame {
...
// GetBufferUsage returns the current buffer usage (number of frames in buffer)
func (e *FrameExtractor) GetBufferUsage() int {
	return len(e.frameBuffer)
}
...
// SetOnFrame sets the callback for extracted frames
func (e *FrameExtractor) SetOnFrame(callback func(*cctvtypes.Frame)) {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.onFrame = callback
}
...
func (e *FrameExtractor) ExtractFrameNow(input string, cameraID string) (*cctvtypes.Frame, error) {
	frame, err := e.extractSingleFrame(input, cameraID)
	if err != nil {
		return nil, err
	}
