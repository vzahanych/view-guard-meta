...
type FrameDistributor struct {
	...
	onFrame    func(*cctvtypes.Frame)     // Callback for frames to send to AI
	mu         sync.RWMutex
	ctx        context.Context
...
type FrameDistributorConfig struct {
	OnFrame func(*cctvtypes.Frame) // Callback for frames to send to AI service
}

// NewFrameDistributor creates a new frame distributor
func NewFrameDistributor(config FrameDistributorConfig, log *zap.Logger) *FrameDistributor {
...
func (d *FrameDistributor) AddExtractor(cameraID string, extractor *FrameExtractor) {
	...
	extractor.SetOnFrame(func(frame *cctvtypes.Frame) {
	...
	d.logger.Info("Frame extractor added", zap.String("camera_id", cameraID))
}
...
func (d *FrameDistributor) RemoveExtractor(cameraID string) {
	...
	if extractor, ok := d.extractors[cameraID]; ok {
		...
		d.logger.Info("Frame extractor removed", zap.String("camera_id", cameraID))
	}
}
...
func (d *FrameDistributor) StopExtraction(cameraID string) {
	...
	if ok {
		extractor.Stop()
		d.logger.Info("Frame extraction stopped", zap.String("camera_id", cameraID))
	}
}
...
func (d *FrameDistributor) GetFrameStats(cameraID string) (FrameStats, error) {
	...
	stats := FrameStats{
		...
		BufferUsage:     extractor.GetBufferUsage(),
	}
