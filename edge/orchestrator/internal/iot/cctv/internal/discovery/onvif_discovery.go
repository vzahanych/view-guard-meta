...
func (s *ONVIFDiscoveryService) SetEventBus(bus eventbus.EventBus) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.eventBus = bus
}
...
func (s *ONVIFDiscoveryService) Stop(ctx context.Context) error {
	s.logger.Info("Stopping ONVIF discovery service")
...
func (s *ONVIFDiscoveryService) discoverCameras() {
	s.logger.Info("Starting ONVIF camera discovery on local network")

	// WS-Discovery probe for ONVIF devices
	devices, err := s.wsDiscoveryProbe()
	if err != nil {
		s.logger.Error("WS-Discovery probe failed", zap.Error(err))
		return
	}
	...
	for _, device := range devices {
		...
		if err != nil {
			s.logger.Debug("Failed to probe camera", zap.String("device", device), zap.Error(err))
			continue
		}
		...
		if existing, ok := s.discoveredCameras[camera.ID]; ok {
			...
			s.logger.Info("Discovered new camera",
				...
				zap.String("ip", camera.IPAddress),
			)
		}
	}

	s.logger.Info("ONVIF discovery complete", zap.Int("cameras_found", len(devices)))
}
...
func (s *ONVIFDiscoveryService) wsDiscoveryProbe() ([]string, error) {
	...
	if err != nil {
		s.logger.Debug("Could not determine local interface, using default", zap.Error(err))
