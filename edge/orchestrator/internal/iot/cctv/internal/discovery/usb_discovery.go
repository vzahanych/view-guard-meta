...
func NewUSBDiscoveryService(discoveryInterval time.Duration, videoDevPath string, log *zap.Logger) *USBDiscoveryService {
...
func (s *USBDiscoveryService) SetEventBus(bus eventbus.EventBus) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.eventBus = bus
}
...
func (s *USBDiscoveryService) Stop(ctx context.Context) error {
	s.logger.Info("Stopping USB camera discovery service")
...
func (s *USBDiscoveryService) discoverCameras() {
	s.logger.Info("Starting USB camera discovery")

	// Find all video devices
	videoDevices, err := s.findVideoDevices()
	if err != nil {
		s.logger.Error("Failed to find video devices", zap.Error(err))
		return
	}
	...
	for usbDevicePath, videoDevices := range usbDeviceMap {
		...
		if err != nil {
			s.logger.Debug("Failed to probe USB device", zap.String("device", primaryDevice), zap.Error(err))
			continue
		}
		...
		if existing, ok := s.discoveredCameras[camera.ID]; ok {
			...
			s.logger.Info("Discovered new USB camera",
				...
				zap.String("device", primaryDevice),
				zap.Strings("devices", videoDevices),
	...
	for id, cam := range s.discoveredCameras {
		...
		if !devicePresent {
			...
			s.logger.Info("USB camera disconnected", zap.String("id", id))
		}
	}

	s.logger.Info("USB discovery complete", zap.Int("cameras_found", len(s.discoveredCameras)))
}
